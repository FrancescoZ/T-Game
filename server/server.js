...
//var args = {x : 0, y : 0, h : config.gameHeight, w : config.gameWidth, maxChildren : 1, maxDepth : 5};
//console.log(args);
...
function tickPlayer(currentPlayer) {
        ...
        for(var i=0; i<user.cells.length; i++) {
            if(user.cells[i].mass > 10 && user.id !== currentPlayer.id) {
                ...
                var collided = SAT.testCircleCircle(playerCircle,
                    new config(new V(user.cells[i].x, user.cells[i].y), user.cells[i].radius),
                    response);
                if (collided) {
    ...
    for(var z=0; z<currentPlayer.cells.length; z++) {
        ...
        var playerCircle = new config(
            new V(currentCell.x, currentCell.y),
            currentCell.radius
        );

        var foodEaten = food.map(funcFood)
            .reduce( function(a, b, config) { return b ? a.concat(config) : a; }, []);
